...
def run_algorithm():
    ...
    try:
        ...
        data_path = data_path = Path(upload_folder) / file.filename
...
def get_image():
    ...
    """      
    img1_static_path = f"/static/images/{Path(img1_path).name}"
    img2_static_path = f"/static/images/{Path(img2_path).name}"

 
    try:
        return jsonify({"img1": f"{img1_static_path}"},  #başka bir çözüm yolu var mı ?
                    {"img2": f"{img2_static_path}"}), 200 
    except Exception as e:
